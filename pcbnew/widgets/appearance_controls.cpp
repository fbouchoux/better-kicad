...
static uint32_t colorfulNameHash( const wxString& aName )
{
    ...
    for( wxUniChar ch : aName )
        hash = ( hash ^ static_cast<uint32_t>( ch.GetValue() ) ) * 16777619u;
...
static COLOR4D colorfulNetColor( const NETINFO_ITEM& aNet )
{
    ...
    if( aNet.HasAutoGeneratedNetname() )
        return COLOR4D( 0.12, 0.76, 0.27, 1.0 );

    if( name.StartsWith( wxT( "+" ) ) )
    {
        // Accept both +3.3V and the common +3V3 spelling.
        wxString number;
        ...
        for( size_t i = 1; i < name.length(); ++i )
        {
            ...
            if( ch >= '0' && ch <= '9' )
                number += ch;
            else if( ( ch == '.' || ch == 'V' || ch == 'v' ) && !hasDecimalPoint
                     && i + 1 < name.length() && name[i + 1] >= '0' && name[i + 1] <= '9' )
            {
                number += '.';
        ...
        double voltage = 0.0;
        number.ToDouble( &voltage );

        COLOR4D color;
        color.FromHSV( 55.0 - 33.0 * voltage / ( voltage + 12.0 ), 0.9, 0.98 );
        return color;
    }

    // Use up to two underscore-separated groups for the main hue.  This keeps
    // NDP_AUD_* separate from NDP_SPI_* while their individual signals stay close.
    wxString family;
    int firstSeparator = name.Find( '_' );

    if( firstSeparator == wxNOT_FOUND )
        family = name.Left( 3 );
    else
    {
        int secondSeparator = name.Mid( firstSeparator + 1 ).Find( '_' );
        int familyLength = secondSeparator == wxNOT_FOUND ? firstSeparator
                                                          : firstSeparator + 1 + secondSeparator;
        family = name.Left( familyLength );
    }

    uint32_t hash = colorfulNameHash( family );
    constexpr double bands[][2] = { { 177.0, 255.0 }, { 270.0, 345.0 }, { 345.0, 372.0 } };
    const double* band = bands[hash % 3];
    double hue = band[0] + 13.0 + ( band[1] - band[0] - 26.0 )
                                  * ( ( hash / 3 ) % 10000 ) / 9999.0;
    double offset = 0.0;
    wxString suffix = name.Mid( family.length() );
    double weight = 4.0;

    for( wxUniChar ch : suffix )
    {
        offset += ( static_cast<int>( ch.GetValue() % 29 ) - 14 ) * weight / 14.0;
        weight *= 0.5;
    }

    hue += 12.0 * std::tanh( offset / 3.0 );

    if( hue >= 360.0 )
        hue -= 360.0;

    uint32_t shade = colorfulNameHash( suffix );
    COLOR4D color;
    color.FromHSV( hue, 0.72 + ( shade % 5 ) * 0.035,
                        0.88 + ( shade % 7 ) * 0.015 );
    return color;
}
...
void APPEARANCE_CONTROLS::createControls()
{
    ...
    m_cbColorfulMode = new wxCheckBox( netDisplayPane, wxID_ANY, _( "Colorful mode" ) );
    m_cbColorfulMode->SetFont( infoFont );
...
void APPEARANCE_CONTROLS::updateColorfulNetColors()
{
    BOARD* board = m_frame->GetBoard();
    ...
    for( NETINFO_ITEM* net : board->GetNetInfo() )
    {
        if( net->GetNetCode() > 0 && !net->GetNetname().IsEmpty() )
            colors.emplace( net->GetNetCode(), colorfulNetColor( *net ) );
    }

    KIGFX::RENDER_SETTINGS* rs = m_frame->GetCanvas()->GetView()->GetPainter()->GetSettings();
    static_cast<KIGFX::PCB_RENDER_SETTINGS*>( rs )->SetColorfulNetColors( std::move( colors ) );
}


void APPEARANCE_CONTROLS::onColorfulMode( wxCommandEvent& aEvent )
{
    KIGFX::RENDER_SETTINGS* rs = m_frame->GetCanvas()->GetView()->GetPainter()->GetSettings();
